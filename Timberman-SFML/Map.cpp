...
void Map::LoadMap(const std::string& l_path){
	...
	while(std::getline(mapFile,line)){
		...
		if(type == "TILE"){
			...
			int entityId = m_context->entityManager->AddEntity(name);
			if (entityId < 0){ continue; }
			if(name == "Player"){ m_playerId = entityId; }
			C_Base* position = m_context->entityManager->GetComponent<C_Position>(entityId,Component::Position);
...
void Map::Draw(unsigned int l_layer){
	...
	sf::RenderWindow* l_wind = m_context->window->GetRenderWindow();
